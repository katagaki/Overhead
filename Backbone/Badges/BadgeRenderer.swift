...
private func layerView(_ layer: BadgeLayer, side: CGFloat,
    ...
    // Emit the real primitives: SwiftUI draws its built-ins differently from a
    // custom Shape returning the same path.
    Group {
        switch layer.shape {
        case "circle":
        ...
        default:
            // The bare initialiser's default corner style is not .circular on
...
public struct LineBadge: View {
    ...
    public init(symbol: String, color: Color, dimension: CGFloat = 32,
                spec: BadgeStyleSpec, overrides: [String: String]? = nil) {
        self.symbol = symbol
        self.color = color
...
public struct StationBadge: View {
    let code: String
    let color: Color
    ...
    public init(code: String, color: Color, opacity: Double = 1.0, side: CGFloat = 28,
                spec: BadgeStyleSpec, overrides: [String: String]? = nil) {
        self.code = code
        self.color = color
...
private func swiftUIWeight(_ name: String?) -> Font.Weight? {
    switch name {
